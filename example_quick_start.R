...
cat("Data dimensions:", nrow(X_train), "x", ncol(X_train), "\n")
cat("Response distribution:", table(y_train), "\n")
cat("Number of true signal variables:", sum(data$beta_true != 0), "\n\n")
...
result <- my_stabsel_parallel(
  ...
  args.fitfun = list(
    groups = groups,
    L_sparse = L,
    alpha_seq = seq(0, 1, 0.25),  # Mixing ratio between Lasso and Laplacian
    nlambda = 10
  ),
  cutoff = 0.6,           # Selection threshold
  pfer_values = c(5),     # Per-Family Error Rate
  sampling.type = "SS",   # Stability Selection
  B = 20,                 # Number of subsampling iterations (for quick execution)
  mc.cores = 4,           # Number of parallel processing cores
  verbose = TRUE
)
...
cat("\nExecution complete! For more detailed analysis, see simulation_study.R.\n")
